...
export default function MapaPage() {
  ...
  if (loading) return (
    <div className="flex h-[60vh] items-center justify-center">
      <p className="text-muted-foreground animate-pulse">{t("mapa.cargando")}</p>
    </div>
  );
  ...
  if (plan !== "PRO" && plan !== "BOSS") {
    return (
      <div className="max-w-3xl mx-auto space-y-6 pb-12">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">{t("mapa.titulo")}</h1>
          <p className="text-muted-foreground mt-1">{t("mapa.subtituloPromo")}</p>
        </div>
        <Card className="border-none shadow-2xl bg-zinc-950 text-white overflow-hidden">
          <CardContent className="p-8 md:p-12 text-center space-y-6">
            ...
            <div>
              <h2 className="text-2xl md:text-3xl font-black">{t("mapa.promoTitulo")}</h2>
              <p className="text-zinc-400 mt-3 text-base max-w-md mx-auto">{t("mapa.promoDesc")}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 text-left max-w-lg mx-auto">
              {[
                { title: t("mapa.feat1Titulo"), desc: t("mapa.feat1Desc") },
                { title: t("mapa.feat2Titulo"), desc: t("mapa.feat2Desc") },
                { title: t("mapa.feat3Titulo"), desc: t("mapa.feat3Desc") },
              ].map((item, i) => (
            ...
            <a href="/dashboard/suscripcion">
              <Button className="bg-white text-black hover:bg-zinc-200 font-black text-base h-12 px-10 mt-4">{t("mapa.verPlanes")}</Button>
            </a>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-12">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl md:text-4xl font-extrabold tracking-tight">{t("mapa.titulo")}</h1>
          <p className="text-muted-foreground mt-1">{t("mapa.subtitulo")}</p>
        </div>
      ...
      {lat && lng ? (
        <Card className="border-border/50 shadow-sm overflow-hidden">
          <CardHeader className="pb-2">
            <CardTitle className="text-base font-bold">{t("mapa.tuUbicacion")}</CardTitle>
          </CardHeader>
      ...
      <Card className="border-border/50 shadow-sm">
        <CardHeader className="pb-3">
          <CardTitle className="text-base font-bold">{t("mapa.configurarTitulo")}</CardTitle>
          <CardDescription>{t("mapa.configurarDesc")}</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={guardar} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label>{t("mapa.direccion")}</Label>
                <Input placeholder={t("mapa.direccion")} value={direccion} onChange={(e) => setDireccion(e.target.value)} className="h-11" />
              </div>
              <div className="space-y-1.5">
                <Label>{t("mapa.ciudad")}</Label>
                <Input placeholder={t("mapa.ciudad")} value={ciudad} onChange={(e) => setCiudad(e.target.value)} className="h-11" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label>{t("mapa.latitud")}</Label>
                <Input placeholder="-34.906" value={lat} onChange={(e) => { setLat(e.target.value); setMapaKey(prev => prev + 1); }} className="h-11" />
              </div>
              <div className="space-y-1.5">
                <Label>{t("mapa.longitud")}</Label>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl border border-border/50 bg-muted/20">
              <div>
                <p className="font-bold text-sm">{t("mapa.visibleLabel")}</p>
                <p className="text-xs text-muted-foreground">{t("mapa.visibleDesc")}</p>
              </div>
            ...
            <Button type="submit" className={`w-full h-12 font-bold transition-all ${guardado ? "bg-green-600 hover:bg-green-700 text-white" : ""}`} disabled={guardando}>
              {guardando ? t("mapa.guardando") : guardado ? t("mapa.guardado") : t("mapa.guardar")}
            </Button>
          </form>
        </CardContent>
      </Card>
