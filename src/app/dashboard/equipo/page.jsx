...
export default function EquipoPage() {
  ...
  const cargarDatos = async () => {
    ...
    const { data: bshop } = await supabase.from("barbershops").select("id, plan, codigo").eq("owner_id", user.id).single();
    if (bshop) {
  ...
  const cargarEquipo = async (bshopId) => {
    ...
    const { data: miembros } = await supabase.from("barbers").select("*").eq("barbershop_id", bshopId).order("created_at", { ascending: true });
    if (!miembros) { setLoadingEquipo(false); return; }

    const equipoConStats = await Promise.all(miembros.map(async (miembro) => {
      if (!miembro.atiende_clientes) return { ...miembro, turnosMes: 0, ingresosMes: 0, turnosHoy: 0 };
      const { data: turnosMes } = await supabase.from("appointments").select("status, services(price)").eq("barber_member_id", miembro.id).gte("start_time", inicioMes);
      const validos = (turnosMes || []).filter(t => t.status !== "falto");
      const hoy = new Date(); hoy.setHours(0,0,0,0);
      const manana = new Date(hoy); manana.setDate(manana.getDate() + 1);
      const { data: turnosHoyData } = await supabase.from("appointments").select("id").eq("barber_member_id", miembro.id).gte("start_time", hoy.toISOString()).lt("start_time", manana.toISOString());
      return { ...miembro, turnosMes: validos.length, ingresosMes: validos.reduce((s, t) => s + (t.services?.price || 0), 0), turnosHoy: (turnosHoyData || []).length };
    }));
  ...
  const cargarSolicitudes = async (bshopId) => {
    const { data } = await supabase.from("invitaciones").select("*").eq("barbershop_id", bshopId).eq("estado", "pendiente").order("created_at", { ascending: true });
    setSolicitudes(data || []);
  };

  const aprobarSolicitud = async (solicitud) => {
    setProcesando(solicitud.id);
    const { error } = await supabase.from("barbers").insert([{ barbershop_id: solicitud.barbershop_id, user_id: solicitud.user_id, name: solicitud.user_name, email: solicitud.user_email, rol: "barber", tipo: "empleado", atiende_clientes: true, activo: true, color: "#2563eb" }]);
    if (error) { alert("Error: " + error.message); setProcesando(null); return; }
    await supabase.from("invitaciones").update({ estado: "aprobada", updated_at: new Date().toISOString() }).eq("id", solicitud.id);
  ...
  const rechazarSolicitud = async (solicitud) => {
    setProcesando(solicitud.id);
    await supabase.from("invitaciones").update({ estado: "rechazada", updated_at: new Date().toISOString() }).eq("id", solicitud.id);
    setSolicitudes(prev => prev.filter(s => s.id !== solicitud.id));
    setProcesando(null);
  };
  ...
  const abrirEdicion = (m) => { setEditandoId(m.id); setEditNombre(m.name); setEditColor(m.color || "#18181b"); setEditAtiende(m.atiende_clientes); };

  const guardarEdicion = async () => {
    setGuardandoEdicion(true);
    const { error } = await supabase.from("barbers").update({ name: editNombre.trim(), color: editColor, atiende_clientes: editAtiende }).eq("id", editandoId);
    if (!error) { setEquipo(prev => prev.map(m => m.id === editandoId ? { ...m, name: editNombre.trim(), color: editColor, atiende_clientes: editAtiende } : m)); setEditandoId(null); }
    else alert("Error: " + error.message);
            ...
            <div className="space-y-3">
              {equipo.map((miembro) => (
                <div key={miembro.id} className={`border rounded-xl overflow-hidden transition-colors ${!miembro.activo ? "opacity-50" : ""}`}>
                  <div className="flex items-center justify-between p-4 cursor-pointer hover:bg-muted/20 transition-colors" onClick={() => editandoId === miembro.id ? setEditandoId(null) : abrirEdicion(miembro)}>
                    <div className="flex items-center gap-3">
