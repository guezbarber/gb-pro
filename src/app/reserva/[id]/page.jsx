...
export default function ReservaPublicaPage({ params }) {
  ...
  const cargarDatosBarberia = async () => {
    ...
    if (error || !cfg) {
      setErrorCarga("No se encontró esta barbería. Verificá el enlace.");
      return;
    }
    setConfig(cfg);
  ...
  if (!config) return (
    <div className="min-h-screen flex items-center justify-center bg-muted/20">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-muted-foreground text-sm">Cargando barbería...</p>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-muted/20 flex flex-col justify-between">
      ...
      <div className="w-full bg-background border-b border-border/40 py-3 px-4 flex justify-center sm:justify-start items-center">
      ...
      <div className="flex-1 p-4 md:p-8 flex justify-center items-start">
        <Card className="w-full max-w-lg border-border/50 shadow-xl mt-4 sm:mt-10 overflow-hidden">
          ...
          <div className="bg-zinc-950 p-6 md:p-8 text-center">
              ...
              <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4">
                ...
                <div className="grid gap-3">
                  ...
                  <button onClick={() => { setBarberoElegido(null); setPaso(2); }}
                    ...
                    <p className="text-muted-foreground font-medium text-sm">Sin preferencia — cualquier barbero disponible</p>
                  </button>
                </div>
              </div>
            )}
                  ...
                  {servicios.map((svc) => (
                    <button key={svc.id} onClick={() => { setServicioElegido(svc); setPaso(3); }}
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-0.5">{svc.duration_minutes} min</p>
                      </div>
            ...
            {paso === 2 && tieneEquipo && (
              <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                <div className="flex justify-between items-center mb-4">
                  <h2 className="text-xl font-bold">2. Elige tu servicio</h2>
                  <button onClick={() => setPaso(1)} className="text-sm text-muted-foreground hover:text-foreground font-medium">Volver</button>
                </div>
                {barberoElegido && (
            ...
            {paso === 3 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="space-y-3 pt-2">
                  ...
                  {cargandoHoras ? (
                    <p className="text-sm text-muted-foreground text-center py-6 animate-pulse">Buscando horarios...</p>
                    ...
                    <p className="text-sm text-destructive font-medium p-4 bg-destructive/10 rounded-lg text-center border border-destructive/20">
                      Agenda llena para este día. Prueba con otro.
                    </p>
                  ) : (
            ...
            {paso === 4 && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
                ...
                <div className="bg-muted/20 p-5 rounded-xl border border-border/50 text-sm space-y-2">
                  ...
                  <p className="flex justify-between"><span className="text-muted-foreground">Servicio:</span><span className="font-bold">{servicioElegido?.name}</span></p>
                  <p className="flex justify-between"><span className="text-muted-foreground">Total:</span><span className="font-black text-lg">${servicioElegido?.price}</span></p>
                  {requiereSena && (
                    <p className="flex justify-between text-amber-700 font-bold border-t border-border/50 pt-2 mt-2">
                      <span>Seña a pagar ahora ({porcentajeSena}%):</span>
                      <span>${montoSena}</span>
                    </p>
                  )}
                ...
                <form onSubmit={confirmarReserva} className="space-y-4 pt-2">
                  <div className="space-y-2">
                    ...
                    <Input required placeholder="Ej: Lucas" className="h-14 text-lg" value={nombre} onChange={(e) => setNombre(e.target.value)} />
                  </div>
                  <div className="space-y-2">
                    <Label className="text-base">Teléfono / WhatsApp</Label>
                    <Input required type="tel" placeholder="Ej: 099123456" className="h-14 text-lg" value={telefono} onChange={(e) => setTelefono(e.target.value)} />
                  </div>
                  <div className="space-y-2">
          ...
          <div className="bg-muted/30 py-4 text-center border-t border-border/50">
            <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest">
              Powered by <span className="font-black text-primary">GB PRO</span>
            </p>
          </div>
        </Card>
      </div>
